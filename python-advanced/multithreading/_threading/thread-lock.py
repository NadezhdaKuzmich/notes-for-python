...
def producer():
    ...
    with lock:
        print('done')
        # Спроба взяти блокування в рамках поточного потоку дає нам DEAD LOCK
        # З цього блокування не вийти, оскільки ми очікуємо завершення
        # самого себе, щоб взяти блокування-блокування ніколи не
        # звільниться.
...
# блокування, що дозволяє зазначити до якої ділянку коду атомарним.
lock = threading.Lock()
