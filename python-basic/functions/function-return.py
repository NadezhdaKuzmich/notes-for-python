# Для того, щоб повернути результат функції, потрібно скористатися спеціальним словом return.
def sum_numbers(a, b):
    return a + b
...
# Задачки:
""" 1. Знайдіть максимальне значення в списку, що передається в функцію, і поверніть його, якщо воно більше 0, в іншому
випадку поверніть повідомлення про те, що число менше 0. """
# v1
...
max_b = get_max_from_list([-1, -2, -7, -9, -6])  # => Max value is less then zero
print(max_a)
print(max_b)
...
""" 3. Напишіть функцію, яка зводить до ступеня число, яке передається у параметрах, якщо ступінь
не є негативним. Перший параметр - число, другий – ступінь, у якому його необхідно звести. """
# v1
