...
# скорочується дублювання коду, що дуже важливо для хорошого та підтримуваного проекту.
# Поліморфізм досягається 2 методами:
# - Перевантаження:
# Перевантаження методу або overloading - це заздалегідь визначений алгоритм усередині самого методу, завдяки
# якому при різній комбінації задіяних аргументів функція веде себе по-різному.
# Перевантажуючи метод, ви заздалегідь визначаєте аргументи, у разі використання яких, функція поводиться інакше.
class Base:
    def __init__(self, a):
        self.__a = a
...
# Ви не зможете перевизначити аргументи методу дочірнього класу (тільки якщо це не конструктор).
# Але ви можете змінювати внутрішню логіку самого методу (це і є перевизначення або overwriting).
class Multiplier:
    def __init__(self, a):
        self._a = a
...
class Car:
    ...
    def distance_on_max_speed(self, distance):
        if self._max_speed == 0:
            print(
                f"Speed = 0, select valid car brand: {list(self._speed_name_map.keys())}"
            )
            return 0
